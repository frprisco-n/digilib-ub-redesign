...
if (startIndex === -1) {
  throw new Error('Could not find allBooks header');
}
const arrayStart = text.indexOf('[', startIndex);
const arrayEnd = text.indexOf('];', arrayStart);
if (arrayStart === -1 || arrayEnd === -1) {
  throw new Error('Could not find allBooks array bounds');
}
...
for (let i = 0; i < arrayText.length; i++) {
  ...
  if (ch === '{') {
    ...
    if (depth === 0 && currentStart !== null) {
      blocks.push({ start: currentStart, end: i, text: arrayText.slice(currentStart, i + 1) });
      currentStart = null;
    }
  }
}
...
function makeDescription(title, author, genre) {
  ...
  if (lower.includes('bumi')) return 'Novel fiksi jalan cerita yang menggabungkan persahabatan, konflik, dan pencarian arti hidup.';
  ...
  if (lower.includes('pulang') && author.toLowerCase().includes('leila')) return 'Kumpulan cerita pendek atau novel yang mengangkat tema nostalgia, cinta, dan sosial budaya Indonesia.';
  ...
  if (genre === 'Sejarah') return `Buku sejarah yang mengulas peristiwa, tokoh, dan dinamika sosial secara naratif.`;
  if (genre === 'Sains') return `Buku sains yang menjelaskan konsep ilmiah utama dan relevansinya dalam kehidupan sehari-hari.`;
  if (genre === 'Teknologi') return `Buku teknologi yang membahas prinsip modern dan penerapan praktisnya.`;
...
for (const block of blocks) {
  ...
  insertions.push({
    position: block.start + yearMatch.index + yearMatch[0].length,
